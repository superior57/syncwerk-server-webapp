...
@Component({
  selector: 'app-upload-links',
  templateUrl: './upload-links.page.html',
  styleUrls: ['./upload-links.page.scss']
})
export class UploadLinksPageComponent implements OnInit {
  ...
  getLinkList(): Promise<any> {
    return new Promise((resolve) => {
      this.shareAdminService.getAllUploadLinks().subscribe(resps => {
  ...
  getLink(linkObj) {
    // return this.filesService.getURLByPath(`share-link/u/d/${linkObj.token}`);
  ...
  viewLink(link: string) {
    ...
    jQuery('#view-link-modal').modal('show');
  }
  ...
  handleOpenModal(modalName: string, currentItem: any) {
    this.currentShareLink = currentItem;
    if (modalName === 'remove') {
  ...
  onRemoveItem() {
    this.shareAdminService.removeSharedUploadLink(this.currentShareLink.token)
      .subscribe(resps => {
        this.loadData();
