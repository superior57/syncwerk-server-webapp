...
export class MeetingRecordingComponent implements OnInit {
  ...
  getListRecording() {
    this.meetingsService.getListRecording(this.meetingId).subscribe(resp => {
  ...
  publishRecording(recording: any, isPublish: string) {
    ...
    this.meetingsService.publishRecording(recording.meeting_id, recording.record_id, isPublish).subscribe(resp => {
      this.getListRecording();
