...
@Component({
  selector: 'app-add-import-users-modal',
  templateUrl: './add-import-users-modal.component.html',
  styleUrls: ['./add-import-users-modal.component.scss']
})
export class AddImportUsersModalComponent implements OnInit {
  ...
  addUser() {
    ...
    this.adminService.postAddUser(newUserInfo).subscribe(resp => {
      this.onAddUserSuccess.emit(resp);
    }, error => {
      this.isAddProcessing = false;
      const errorBody = JSON.parse(error._body);
      const errorMessageHtml = JSON.parse(errorBody.data);
      const errorHTMLElement = jQuery(errorMessageHtml.error);
      this.noti.showNotification('danger', errorHTMLElement[0].textContent);
    });
  }
  ...
  onFileChange(data) {
    ...
    if (selectedFile.type !== 'text/csv' && selectedFile.type !== 'application/vnd.ms-excel') {
      this.clearFileInput();
      this.noti.showNotification('danger', this.translate.instant('NOTIFICATION_MESSAGE.ONLY_CSV_FILES_ALLOWED'));
      return;
    }
  ...
  importUsers() {
    ...
    this.adminService.postImportUser(this.selectedFile).subscribe(resp => {
      this.onAddUserSuccess.emit(resp);
    }, err => {
