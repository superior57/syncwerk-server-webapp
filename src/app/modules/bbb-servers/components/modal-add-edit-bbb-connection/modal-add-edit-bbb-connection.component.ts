...
import { NotificationService, BBBService, MeetingsService } from 'app/services';
import { BsModalRef } from 'ngx-bootstrap/modal/bs-modal-ref.service';
import { TranslateService } from '@ngx-translate/core';

@Component({
  selector: 'app-modal-add-edit-bbb-connection',
  templateUrl: './modal-add-edit-bbb-connection.component.html',
  styleUrls: ['./modal-add-edit-bbb-connection.component.scss']
})
export class ModalAddEditBbbConnectionComponent implements OnInit {
  ...
  ngOnInit() {
    if (this.isEditMode) {
      this.bbbService.getBBBServerById(this.selectedServerId).subscribe(resp => {
  ...
  addBBBServer() {
    ...
    this.bbbService.postAddBBBServer(this.bbb_form).subscribe(resp => {
  ...
  editBBBServer() {
    ...
    this.bbbService.putEditBBBServer(this.bbb_form.id, this.bbb_form).subscribe(resp => {
      this.notify.showNotification('success', resp.message);
      this.closeModal();
    }, error => {
      this.notify.showNotification('danger', 'Server error');
